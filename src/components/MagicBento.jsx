...
import { MessageSquare, FileText, BrainCircuit, Layers, Map, Edit3, PieChart, ShieldCheck } from 'lucide-react';
...
const BentoCardGrid = ({ children, gridRef, gridClassName = "card-grid" }) => (
  <div className={`${gridClassName} bento-section`} ref={gridRef}>
    {children}
  </div>
);
...
const MagicBento = ({
  ...
  // Disable heavy animations completely on mobile (stars, spotlight, border glow)
  // We keep click ripples and basic hover if not strictly disabled
  const shouldDisableHeavy = isMobile; 
  const shouldDisableAnimations = disableAnimations || isMobile;

  return (
    <>
      {enableSpotlight && !shouldDisableHeavy && (
        <GlobalSpotlight
          gridRef={gridRef}
      ...
      <BentoCardGrid gridRef={gridRef} gridClassName={gridClassName}>
        {cards.map((card, index) => {
          const baseClassName = `magic-bento-card ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow && !shouldDisableHeavy ? 'magic-bento-card--border-glow' : ''}`;
          const cardProps = {
          ...
          if (enableStars && !shouldDisableHeavy) {
            return (
              <ParticleCard
                key={index}
                {...cardProps}
                ...
                onClick={() => { if(card.onClick) card.onClick(); else if(card.route) setCurrentPage(card.route); }}
              >
                <div className="magic-bento-card__header">
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div className="magic-bento-card__icon-wrapper" style={{ display: 'flex', color: 'var(--white)' }}>
          ...
          return (
            <div
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  if(card.onClick) card.onClick(); else if(card.route) setCurrentPage(card.route);;
                }
              }}
              ref={el => {
                if (!el) return;
